...
exports.getHabits = async (req, res) => {
  try {
    ...
    // Traer últimos 90 días para el calendario para no saturar memoria
    const logsResult = await db.query(
      "SELECT * FROM habit_logs WHERE log_date >= CURRENT_DATE - INTERVAL '90 days' ORDER BY log_date DESC"
    );
...
exports.createHabit = async (req, res) => {
  try {
    ...
    const newHabit = await db.query(
      ...
      [
        title, description || '', frequency_type || 'daily', frequency_count || 1, color || 'var(--primary)',
...
exports.updateHabit = async (req, res) => {
  try {
    ...
    const updateHabit = await db.query(
      ...
      [
        title, description, frequency_type, frequency_count, color,
        targetDaysStr, finalType, target_value || 1, unit || '', icon || '🎯', reminder_time || null, id
      ]
    );
...
exports.toggleHabit = async (req, res) => {
  try {
    ...
    const habitResult = await db.query('SELECT frequency_count, type, target_value FROM habits WHERE id = $1', [id]);
